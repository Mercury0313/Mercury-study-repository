...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, x, target):
        """
        Args:
            x: 模型输出 (batch, classes)
            target: 真实标签 (batch, classes) - one-hot 编码
        """
...
def train_model(model, train_loader, val_loader, epochs=50, lr=0.001, device='cuda', fold=None, patience=10, weight_decay=1e-4, label_smoothing=0.1):
    """
    训练模型
    Args:
        model: 模型
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    plt.savefig('k0_performance.png', dpi=150)
    plt.show()
